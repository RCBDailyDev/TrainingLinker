...
class DataSetTrainingLinkerLora(TabBase):
    def __init__(self, standalone=False):
        super().__init__()
        self.root_path = ""
        self.tab_name = "DataSetTrainingLinkerLora"
        self.standalone = standalone
        self.cmd_executor = train_linker_executor.CommandExecutor()
        if standalone:
            self.root_path = os.getcwd() + "/TrainDataPath"
        self.ui_kv = {}
        self.cfg = train_linker_cfg_mgr.TrainCfgLora()
        self.create_tab_ui()
    ...
    def create_tab_ui(self):
        with gr.Group():
            with gr.Accordion("CFG Manager") as self.cfg_manager:
                with gr.Row():
                    self.dd_cfg_list = gr.Dropdown(label=dsmc.LORA_CFG, choices=[], interactive=True)
            ...
            with gr.Row() as self.row_detail:
                ...
                with gr.Column():
                    with gr.Row():
                        with gr.Row():
                            self.lb_cfg_name = gr.Label(label="CfgName", elem_classes="tl_lora_background")
                    ...
                    with gr.Row():
                        ...
                        self.text_encoder_lr = gr.Number(label="text_encoder_lr", value=1e-6, interactive=True)
                        self.__RegisterUI("text_encoder_lr", self.text_encoder_lr)
                    with gr.Row():
                        self.train_batch_size = gr.Number(label="Train Batch Size", value=4, interactive=True, minimum=1, maximum=10)
                        self.__RegisterUI("train_batch_size", self.train_batch_size)
                        self.network_dim = gr.Number(label="Network Dim", value=128, interactive=True, minimum=16,
                                                     maximum=512, step=1)
                        self.__RegisterUI("network_dim", self.network_dim)
                        self.network_alpha = gr.Number(label="Network Alpha", value=32, interactive=True,
                                                       minimum=0.0001, maximum=1024, step=0.0001)
                        self.__RegisterUI("network_alpha", self.network_alpha)
                    with gr.Row():
    ...
    def impl_ui_logic(self):
        ...
        self.btn_print_cmd.click(lambda: print(data_set_cmd_maker.make_lora_cmd(self.cfg.cfg_obj)))
        ...
        def btn_new_cfg_click():
            ...
            cfg_file_path = os.path.join(self.root_path, dsmc.LORA_CFG, file_name)
        ...
        def btn_load_cfg_click(cfg_name):
            ...
            cfg_file_path = os.path.join(self.root_path, dsmc.LORA_CFG, cfg_name)
        ...
        self.btn_run_api.click(fn=btn_run_api_click, inputs=[self.json_cmd_json], api_name="train_lora")
        ...
        self.btn_stop_standlone.click(fn=lambda: self.cmd_executor.kill_command(), api_name="stop_lora")

        self.btn_open_cfg_path.click(lambda: util.open_folder(os.path.join(self.root_path, dsmc.LORA_CFG), False))
    ...
    def GetCfgList(self, root_path):
        ...
        cfg_path = os.path.join(root_path, dsmc.LORA_CFG)
    ...
    def __CreateDefaultCfg(self, file_name_in="Default", default_path_name=dsmc.LORA_CFG,
